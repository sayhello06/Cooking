...
<div id="sev">
    ...
    for ($i=0; $row=sql_fetch_array($hresult); $i++)
    {
        ...
        for($k=1; $row2=sql_fetch_array($result2); $k++) {
            ...
            echo '<div class="ev_txt_wr"><div class="ev_txt"><a href="'.$item_href.'" class="ev_prd_tit">'.get_text(cut_str($row2['it_name'], 30)).'</a>'.PHP_EOL;
            echo '<span class="ev_prd_price">'.display_price(get_price($row2), $row2['it_tel_inq']).'</span></div></div>'.PHP_EOL;
