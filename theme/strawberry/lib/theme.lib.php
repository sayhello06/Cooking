...
function item_icon2($it)
{
    ...
    $icon = '<span class="sct_icon1">';

    if ($it['it_type4'])
        $icon .= '<span class="icon icon_best">인기</span>';

    if ($it['it_type5'])
        $icon .= '<span class="icon icon_sale">할인</span>';

    if ($it['it_type3'])
        $icon .= '<span class="icon icon_new">신상</span>';
...
function item_icon3($it)
{
    global $g5;

    $icon = '<span class="sct_icon2">';

    if ($it['it_type2'])
        $icon .= '<span class="icon icon_rec">추천</span>';

    if ($it['it_type1'])
        $icon .= '<span class="icon icon_hit">히트</span>';
    ...
    $row = sql_fetch($sql);
    if($row['cnt'])
        $icon .= '<span class="icon icon_cp">쿠폰</span>';
