...
<div id="sev">
    ...
    for ($i=0; $row=sql_fetch_array($hresult); $i++)
    {
        ...
        for($k=1; $row2=sql_fetch_array($result2); $k++) {
            ...
            echo '<span class="ev_prd_img">'.get_it_image($row2['it_id'], 270, 270, get_text($row2['it_name'])).'</span>'.PHP_EOL;
            ...
            echo '<span class="sct_discount">'.display_price($row2['it_cust_price']).'</span>'.PHP_EOL;
            $s_core  =  (int)$row2['it_use_avg']; 
            if ($s_core > 0 ) { 
