...
<div id="sbsk" class="tnb_con">
    <form name="frmcartlist2" method="post" action="<?php echo $cart_action_url; ?>">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $it_img = get_it_image($row['it_id'], 70, 70, true);

        echo '<div class="cart_img prd_img">'.$it_img.'</div>'.PHP_EOL;
        ...
        echo '</div>'.PHP_EOL;
        echo '<button class="cart_del prd_del" type="button" data-it_id="'.$row['it_id'].'">삭제</button>'.PHP_EOL;
        echo '</li>'.PHP_EOL;
    }
    ...
    <div class="cart_btn qk_cart_btn">
        ...
        <a href="<?php echo G5_SHOP_URL; ?>/cart.php" class="qk_go_cart btn_b01">장바구니 가기</a>
    </div>
    </form>
 </div>
<script>
